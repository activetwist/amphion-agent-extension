...
exports.renderCursorRules = renderCursorRules;
function renderAntigravityWorkflow(command, config) {
...
function renderCursorRule(command, config) {
    const description = getCommandDescription(command, config);
    const content = getCommandContent(command);
...
function getCommandDescription(command, config) {
    const cmd = command.toLowerCase();
    if (cmd === 'docs')
        return `Generate Project Strategy (Charter & PRD) from source documents for ${config.projectName}`;
    if (cmd === 'board')
        return `Populate the Command Board and draft contracts based on Evaluate findings for ${config.projectName}`;
...
2. **Observability**: Always write detailed stories (cards) to the Command Deck board (\`ops/launch-command-deck/data/state.json\`) alongside your evaluations and contracts. This allows the Product Manager to read the cards and track progress.
`;
}
