...
4. **Record findings** via the Command Deck API:
   - `GET http://127.0.0.1:{port}/api/conventions?intent=findings` for the payload schema.
   - `POST http://127.0.0.1:{port}/api/milestones/{milestoneId}/artifacts` with `artifactType: findings`.
