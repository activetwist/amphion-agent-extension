...
Resolve the port from `.amphion/config.json` (field: `port`). If config.json is missing, inform the user to run `/amphion`.
...
1. Check if the server is already running: `curl -s http://127.0.0.1:{port}/api/health`
...
2. Start the server:
   ...
   ```
3. Wait 2 seconds, then verify health: `curl -s http://127.0.0.1:{port}/api/health`
4. If healthy, report "Command Deck started on port {port}." and open the dashboard: `open http://127.0.0.1:{port}`
...
3. Wait 1 second, verify: `curl -s http://127.0.0.1:{port}/api/health` should fail.
4. Report "Command Deck stopped."

### `status`

1. Check health: `curl -s http://127.0.0.1:{port}/api/health`
2. If canonical runtime detected, report: "Command Deck online at http://127.0.0.1:{port}"
