...
3. **Verify completion**: `GET http://localhost:{port}/api/find?milestoneId={milestoneId}` — confirm all contract cards are in "Done" list.

4. **Record outcomes artifact**:
   - `GET http://localhost:{port}/api/conventions?intent=outcomes` for schema.
   - `POST http://localhost:{port}/api/milestones/{milestoneId}/artifacts` with `artifactType: outcomes`.

5. **Write closeout memory**:
   - `POST http://localhost:{port}/api/memory/events` with handoff state.
   - Verify via `GET http://localhost:{port}/api/memory/query?key=task.{issueNumber}.handoff`.
