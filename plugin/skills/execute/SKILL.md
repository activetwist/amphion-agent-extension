...
3. **Load approved contracts**: `GET http://127.0.0.1:{port}/api/find?milestoneId={milestoneId}` to verify approved card set exists.

4. **Implement**: Execute changes exactly as authorized by the contract cards. Do not deviate from approved AFPs.

5. **Move cards**: As you begin each card, move it to "In Progress" via `PATCH http://127.0.0.1:{port}/api/cards/{cardId}` with `listId` for the active list. Move to "Done" when complete.
